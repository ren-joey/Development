...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__module_1_js__ = __webpack_require__(126);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__module_2_js__ = __webpack_require__(129);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_jquery__ = __webpack_require__(63);
...
window.onload = function () {
  ...
  Object(__WEBPACK_IMPORTED_MODULE_1__module_2_js__["default"])();

  //babel new variable
  var x = void 0; //block scoped限定變數，只能在這個function中存活
  for (var i = 0; i < 10; i++) {
    x = 'foo';
  }
  var y = 10; //定義變數，不能被改變
  ...
  var target = [1, 2];
  var nums = target.map(function (para, order) {
    return String(para) + ',' + String(order);
  });
  //map 第一個傳入值para是參數定義的值，地案個傳入的是參數的順序
  //map 的作法類似foreach，只是 map 會開啟一個新的 array 再把算好的資料丟進目標李
  //foreach 則是直接改變原本的變數
  __WEBPACK_IMPORTED_MODULE_2_jquery___default()('<div/>', {
    text: nums
  }).appendTo('body');
  ...
  var user = {
    ...
    greet: function greet() {
      ...
      this.friends.forEach(function (f) {
        console.log('Arrow Function: ' + _.join(['Hi', f, ',', 'I\'m', _this.name], ' '));
      });
    }
  ...
  console.log('Spread: ' + spreadFunc.apply(undefined, [5, 2, 3]));
  ...
  var _ref = [1, 2],
      prop1 = _ref[0],
      _ref$ = _ref[2],
      prop2 = _ref$ === undefined ? 3 : _ref$;
  //將右邊的值丟進對應排序的名稱，沒有定義的名稱就略過，沒有值可以丟入的名稱也可以設定預設值
  //prop1 = 1, prop2 = 3  (2被跳過，因為沒有對應名稱，prop2因為沒有值放入變成預設值3)

  function doCareProperty(_ref2) {
    var _ref2$name = _ref2.name,
        name = _ref2$name === undefined ? 'joey' : _ref2$name;
    return name;
  }
  ...
  var tempName = 'Bob',
      tempScore = 3.1416,
      tempStr = tempName + ' gets a score ' + tempScore.toFixed(2);
  ...
  var userClasses = function () {
    //跟 function userClasses() 是一樣的意思
    function userClasses(name, version) {
      _classCallCheck(this, userClasses);

      this.name = name;
      this.version = version;
    }

    _createClass(userClasses, [{
      key: 'greet',
  ...
  var userClassesProp = new userClasses('Bob', '1.1');
  console.log('userClassesProp: ' + userClassesProp.greet());
  console.log('UserStatic: ' + userClasses.yell());

  //inheritance

  var Vip = function (_userClasses) {
    _inherits(Vip, _userClasses);

    //Vip會繼承userClasses所有的內容
    function Vip(name, title) {
      _classCallCheck(this, Vip);

      //super會繼承userClasses對name的處理
      var _this2 = _possibleConstructorReturn(this, (Vip.__proto__ || Object.getPrototypeOf(Vip)).call(this, name));
  ...
  // Iterators
  var arrIterator = [3, 5, 7];
  arrIterator.foo = 'bar';
  console.log('Iterator(in): ');
  for (var _i in arrIterator) {
    // Iterator > in 會依序print出array裡面每一個數值的order
    console.log(_i);
  }
  ...
  try {
    for (var _iterator = arrIterator[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var _i3 = _step.value;
      // Iterator > of 會直接將array裡面的每個數值print出來
      console.log(_i3);
    }
  ...
  var generatorsUser = _defineProperty({
    name: 'Bob',
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            if (!(_i2 < this.friends.length)) {
              _context.next = 6;
              break;
            }

            _context.next = 3;
            return this.friends[_i2];
  ...
  try {
    for (var _iterator2 = generatorsUser[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var _f = _step2.value;

      console.log('Iterators & Generators: ' + _f);
    }
  ...
  var ES7_user = function ES7_user() {
    var _this3 = this;

    _classCallCheck(this, ES7_user);

    this.nickname = 'Andy';
    this.runtimes = 1;

    this.sayHi = function () {
      console.log('Hi, I\'m ' + _this3.nickname + '. runtimes: ' + _this3.runtimes++);
      if (_this3.runtimes >= 10) {
  ...
  ES7_user.privilage = 7;

  ES7_user.canRead = function () {
    return ES7_user.privilage >= 3;
  };

  var ES7_userProp = new ES7_user();
  console.log(ES7_user.canRead());
  var runLoop = setInterval(ES7_userProp.sayHi, 1000);
};
